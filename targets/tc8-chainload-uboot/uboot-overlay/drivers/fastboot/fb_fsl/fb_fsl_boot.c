...
int do_boota(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[]) {
	...
	if ((avb_result == AVB_AB_FLOW_RESULT_OK) ||
		...
		if (boot_header_version == 4) {
			...
			if (!is_android_boot_image_header((void *)hdr)) {	/* TC8: was inverted -> v0 images always failed */
		...
		/* TC8: boot_a carries a combined boot/recovery ramdisk; without
		 * androidboot.force_normal_boot=1 init runs /sbin/recovery and reboots.
		 * Add it BEFORE the (long) dm= avb cmdline so it stays within the kernel's
		 * cmdline length limit — appended at the tail it got truncated and init
		 * never saw it (IsRecoveryMode = !ForceNormalBoot && /system/bin/recovery). */
		if (!is_recovery_mode)
	...
	if (boot_header_version == 4) {
		...
		/* TC8: ALWAYS copy the boot.img ramdisk. SYSTEM_RAMDISK_SUPPORT (forced by
		 * ANDROID_AB) skips it in normal boot, but our images are not system-as-root:
		 * stock needs its first-stage ramdisk, ours is empty. Skipping left a stale
		 * ramdisk at ramdisk_addr with non-zero size -> kernel unpacked garbage -> init -13. */
