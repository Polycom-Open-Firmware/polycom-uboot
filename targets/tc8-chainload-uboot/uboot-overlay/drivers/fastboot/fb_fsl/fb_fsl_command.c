...
/* Board hook: show on the device that it is entering fastboot via the BCB
 * (bootonce-bootloader) path. This runs before bootcmd, so no boot script
 * has probed the display; boards with a panel override this to light it
 * and draw their fastboot indicator. */
__weak void board_fastboot_entry_show(void)
{
}
