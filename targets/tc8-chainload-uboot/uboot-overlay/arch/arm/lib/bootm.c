...
static void announce_and_cleanup(int fake)
{
	...
	/*
	 * TC8 unlock: do NOT video_link_shut_down() at kernel handoff.
	 *
	 * video_link_shut_down() does device_remove() of the lcdif video
	 * device, which cascades imx_sec_dsim_remove() -> remove lcc-panel
	 * plus dsi_host_disable(), and lcdifv3_disable_controller(). With
	 * our F2 bring-up the DSI + LCDIF + lcc-panel pipeline is live (HS
	 * video, splash shown); tearing it down here deadlocks in the
	 * sec-dsim host disable, so stage-2 hangs silently right BEFORE
	 * the "Starting kernel ..." print below. Deterministically
	 * root-caused (diag1-4): byte-identical hang with OF_SYSTEM_SETUP
	 * on or off, all fdt and mmc ops pass, DRAM-GPT-CRC verified;
	 * matches the C60 samsung_dsim teardown-deadlock precedent. Linux
	 * re-inits the display IP from scratch, so leaving the pipeline
	 * running across the jump is benign AND preserves the F2 splash.
	 * Keep CONFIG_VIDEO_LINK enabled (F2 splash + bootsel BMP need
	 * it); only the OS-handoff teardown is skipped.
	 */
