...
int clock_init(void)
{
	...
	/*
	 * Re-point ARM_A53_CLK_ROOT from the sys_pll2_500m safe clock to
	 * arm_pll_out (now locked at 1.2 GHz). CORE_SEL=1 means the CPU
	 * already runs straight off ARM_PLL, so this does not glitch the
	 * running core — but downstream NXP-BSP kernels (e.g. stock Polycom
	 * Android) model the CPU clock *through* arm_a53_src and read this
	 * root. Left at 500 MHz it is an "unlisted" OPP, and their cpufreq
	 * driver hangs forever trying a BD71837 buck2 DVS voltage change.
	 * Pointing it at arm_pll_out makes both clock paths report 1.2 GHz,
	 * a listed OPP, so cpufreq performs no transition.
	 */
	clock_set_target_val(ARM_A53_CLK_ROOT, CLK_ROOT_ON |
			     CLK_ROOT_SOURCE_SEL(1));
