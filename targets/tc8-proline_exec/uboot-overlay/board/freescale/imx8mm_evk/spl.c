...
int power_init_board(void)
{
	...
	/*
	 * Pre-stage BUCK2 (VDD_ARM / cpu arm-supply) at its DVS RUN voltage
	 * of 1.0 V (sel = (1000-700)/10 = 0x1E) to match the stock Polycom
	 * device tree's `bd71837,pmic-buck2-dvs-voltage = <1000000 900000 0>`.
	 */
	 ...
	 * during cpufreq bring-up; if the regs are locked (0x11) those
	 * writes are silently dropped and the driver's readback-verify
	 * loop never converges, deadlocking imx8_register_cpufreq before
	 * userspace. Mainline kernels manage REGLOCK themselves so this
	 * is harmless to our own (kerbek) image.
	 */
	pmic_reg_write(p, BD718XX_REGLOCK, 0x1);
