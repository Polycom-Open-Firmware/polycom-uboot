...
static int sec_mipi_dsim_calc_pmsk(struct sec_mipi_dsim *dsim)
{
	...
	{
		char b[64];
		snprintf(b, sizeof(b), "fin=%u fout=%u p=%u m=%u s=%u",
			 fin, fout, best_p, best_m, best_s);
		env_set("fastboot.pllcalc", b);
	}
...
static int sec_mipi_dsim_config_pll(struct sec_mipi_dsim *dsim)
{
	...
	dsim_write(dsim, 0x1f4, DSIM_PLLTMR);	/* C60: kernel writes 500 */
...
static void sec_mipi_dsim_set_main_mode(struct sec_mipi_dsim *dsim)
{
	...
	mvporch |= MVPORCH_SET_MAINVBP(timings->vback_porch.typ)    |
		   ...
		   MVPORCH_SET_CMDALLOW(0xf);	/* C60: kernel samsung-dsim uses 0xf;
					 * with 0 no HS command ever gets a
					 * blanking slot once video saturates
					 * the link, so post-video DCS (the
					 * 0x51 backlight write) is dropped. */
	...
	/* C60: kernel samsung-dsim programs one wc less for hfp/hbp/hsa
	 * (lit-Linux MHPORCH 0x440010 vs 0x450011 here). Match exactly. */
	mhporch |= MHPORCH_SET_MAINHFP(hfp_wc - 1) |
		   MHPORCH_SET_MAINHBP(hbp_wc - 1);
	...
	msync |= MSYNC_SET_MAINVSA(timings->vsync_len.typ) |
		 MSYNC_SET_MAINHSA(hsa_wc - 1);	/* C60: kernel parity */
					 ...
					 bool enable)
{
	...
	/*
	 * C60: keep StopStateCnt = 0xF (the kernel samsung-dsim idles at
	 * ESCMODE 0x1e00000). Without LP-11 stop-state gaps between HS
	 * bursts the glass drops interleaved command packets once video
	 * runs at the exact link budget.
	 */
	escmode &= ~(0x7ffu << 21);
	escmode |= 0xfu << 21;
...
static void sec_mipi_dsim_config_dphy(struct sec_mipi_dsim *dsim)
{
	...
	/* C60: force the lit-Linux D-PHY timing values for the 477.66 Mbps
	 * link (kernel: 0xb4=0x306, 0xb8=0x2100a04, 0xbc=0x30706 — the NXP
	 * table entry for ~478 MHz differs). */
	dsim_write(dsim, 0x306, DSIM_PHYTIMING);
	dsim_write(dsim, 0x2100a04, DSIM_PHYTIMING1);
	dsim_write(dsim, 0x30706, DSIM_PHYTIMING2);

	timeout = 0xffffff;	/* C60: lit-Linux value (wider LPDR timeout) */
