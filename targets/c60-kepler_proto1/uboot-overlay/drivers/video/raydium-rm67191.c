...
struct rm67191_panel_priv {
	...
	struct gpio_desc power;		/* C60: panel 3.3V enable (gpio2_4) */
	unsigned int lanes;
...
static const struct rad_platform_data rad_c60_kepler;
...
static int rm67191_panel_probe(struct udevice *dev)
{
	...
	/*
	 * C60: stock + our kernel run this glass with NO EoT packets
	 * (kernel mode_flags 0x215 incl. MIPI_DSI_MODE_NO_EOT_PACKET; the
	 * lit-Linux DSIM CONFIG has EOT_R03 set, ours didn't). u-boot's
	 * MIPI_DSI_MODE_EOT_PACKET is the same BIT(9) = "disable EoT".
	 */
	if ((void *)dev_get_driver_data(dev) == &rad_c60_kepler)
		priv->mode_flags |= MIPI_DSI_MODE_EOT_PACKET |
				    MIPI_DSI_MODE_VSYNC_FLUSH;	/* kernel CONFIG bit29 */
	...
	/*
	 * C60: panel 3.3V enable (gpio2_4, active-high). The kernel does
	 * regulator_bulk_enable() -> reset; mirror that here. Optional
	 * (only the C60 node has power-gpio) so other variants are unaffected.
	 */
	ret = gpio_request_by_name(dev, "power-gpio", 0, &priv->power,
				   GPIOD_IS_OUT);
	if (!ret) {
		dm_gpio_set_value(&priv->power, 1);	/* power on */
		mdelay(20);
	}
...
/*
 * --- Poly Trio C60 (Kepler) panel: 720x1280 RM67191-class glass ------------
 * Retargeted from the stock 1080p rm67191. Ground truth = the shipped Linux
 * driver (c60-kernel-patches 0012 / DTS 0001): Himax HX8394-class controller
 * (SETEXTC 0xB9 0xF1 0x12 0x83), 720x1280 @ ~65 Hz, 79.61 MHz, non-burst
 * SYNC-PULSE + HSE. Init = 19 DCS long-writes ("Table B", the FT5x06
 * vendor-id 0x79 calibration variant). Enable order mirrors the kernel
 * rad_panel_enable: push Table B (LP mode) -> exit-sleep -> settle ->
 * display-on; the PWM4 backlight is enabled by the common wrapper.
 * Panel power = gpio2_4 (active-high), reset = gpio2_5 (active-low).
 */
struct c60_init_cmd {
	u8 len;			/* bytes in data[], incl. the leading DCS command */
	u8 data[64];
};

static const struct c60_init_cmd c60_kepler_cmd_set[] = {
	{  4, { 0xB9, 0xF1, 0x12, 0x83 } },
	{ 28, { 0xBA, 0x33, 0x81, 0x05, 0xF9, 0x0E, 0x06, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x44, 0x25, 0x00, 0x91, 0x0A, 0x00, 0x00, 0x02, 0x4F, 0x11, 0x00, 0x00, 0x37 } },
	{  2, { 0xB8, 0x25 } },
	{  4, { 0xBF, 0x02, 0x11, 0x00 } },
	{ 11, { 0xB3, 0x07, 0x0B, 0x1E, 0x1E, 0x03, 0xFF, 0x00, 0x00, 0x00, 0x00 } },
	{ 10, { 0xC0, 0x73, 0x73, 0x50, 0x50, 0x00, 0x00, 0x08, 0x70, 0x00 } },
	{  2, { 0xBC, 0x46 } },
	{  2, { 0xCC, 0x07 } },
	{  2, { 0xB4, 0x80 } },
	{  4, { 0xB2, 0xC8, 0x12, 0x30 } },
	{ 15, { 0xE3, 0x07, 0x07, 0x0B, 0x0B, 0x03, 0x0B, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x80, 0xC0, 0x10 } },
	{ 13, { 0xC1, 0x81, 0x00, 0x35, 0x35, 0x99, 0xF1, 0x7F, 0x7F, 0x3F, 0x3F, 0x3F, 0x3F } },
	{  7, { 0xC6, 0x01, 0x00, 0xFF, 0xFF, 0x00, 0x20 } },
	{  3, { 0xB5, 0x0E, 0x0E } },
	{  3, { 0xB6, 0x3E, 0x3E } },
	{ 64, { 0xE9, 0xC2, 0x10, 0x04, 0x05, 0x00, 0x05, 0xA0, 0x12, 0x31, 0x23, 0x1F, 0x80, 0x05, 0xA0, 0x17, 0x08, 0x0C, 0x00, 0x0C, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x0C, 0x00, 0x00, 0x00, 0x88, 0x64, 0x64, 0x20, 0x88, 0x88, 0x88, 0x88, 0x88, 0x02, 0x88, 0x88, 0x75, 0x75, 0x31, 0x88, 0x88, 0x88, 0x88, 0x88, 0x13, 0x88, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 } },
	{ 62, { 0xEA, 0x00, 0x1A, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x88, 0x13, 0x13, 0x57, 0x88, 0x88, 0x88, 0x88, 0x88, 0x75, 0x88, 0x88, 0x02, 0x02, 0x46, 0x88, 0x88, 0x88, 0x88, 0x88, 0x64, 0x88, 0x23, 0x08, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x05, 0xB0, 0x00, 0x00, 0x00, 0x00 } },
	{ 35, { 0xE0, 0x09, 0x1B, 0x1F, 0x22, 0x1F, 0x2F, 0x48, 0x3D, 0x06, 0x0A, 0x0C, 0x10, 0x12, 0x11, 0x13, 0x16, 0x1B, 0x15, 0x1B, 0x1F, 0x22, 0x1F, 0x2F, 0x48, 0x3D, 0x06, 0x0A, 0x0C, 0x10, 0x12, 0x11, 0x13, 0x16, 0x1B } },
};

static const struct display_timing c60_kepler_timing = {
	.pixelclock.typ		= 79610000,
	.hactive.typ		= 720,
	...
	.vsync_len.typ		= 5,
	/*
	 * PIXDATA POSEDGE (was NEGEDGE): mxsfb sets eLCDIF VDCTRL0.DOTCLK_POL
	 * from the NEGEDGE flag. NEGEDGE made u-boot latch pixel data on the
	 * opposite dot-clock edge from the DSIM sampler (u-boot VDCTRL0
	 * 0x12300005 vs lit-Linux 0x10300005, sole diff = bit25 DOTCLK_POL),
	 * so the panel got correct SYNC but BLACK pixel data -- the real
	 * "black screen, backlight lit" root cause (2026-07-03). Posedge
	 * matches the working Linux mxsfb path.
	 */
	.flags = DISPLAY_FLAGS_HSYNC_LOW |
		 DISPLAY_FLAGS_VSYNC_LOW |
		 DISPLAY_FLAGS_DE_LOW |
...
static int c60_kepler_send_init(struct mipi_dsi_device *dsi)
{
	size_t i;
	int ret;

	for (i = 0; i < ARRAY_SIZE(c60_kepler_cmd_set); i++) {
		const struct c60_init_cmd *e = &c60_kepler_cmd_set[i];

		ret = mipi_dsi_dcs_write(dsi, e->data[0], &e->data[1], e->len - 1);
		if (ret < 0) {
			printf("c60 panel: init cmd %zu (0x%02x) failed (%d)\n",
			       i, e->data[0], ret);
			return -EIO;
		}
	}
	...
	ret = mipi_dsi_dcs_exit_sleep_mode(dsi);
	if (ret < 0) {
		printf("c60 panel: exit sleep failed (%d)\n", ret);
		return -EIO;
	}
	mdelay(120);				/* kernel: sleep-out settle */

	ret = mipi_dsi_dcs_set_display_on(dsi);
	if (ret < 0) {
		printf("c60 panel: display on failed (%d)\n", ret);
		return -EIO;
	}
	mdelay(20);
...
static int c60_kepler_enable(struct udevice *dev)
{
	struct mipi_dsi_panel_plat *plat = dev_get_plat(dev);
	struct mipi_dsi_device *dsi = plat->device;

	/*
	 * Mirror the kernel rad_panel_enable exactly: send the Table-B MCS,
	 * exit-sleep, and display-on all in command/low-power mode, before the
	 * bridge switches the DSIM to video and starts streaming. (The prior
	 * double-init + power-blip + DISBV/readback experiments were
	 * backlight-era dead ends and are removed.)
	 */
	dsi->mode_flags |= MIPI_DSI_MODE_LPM;

	return c60_kepler_send_init(dsi);
}
...
static const struct rad_platform_data rad_c60_kepler = {
	.enable = &c60_kepler_enable,
	.timing = &c60_kepler_timing,
};
...
static const struct udevice_id rm67191_panel_ids[] = {
	...
	{ .compatible = "poly,kepler-c60", .data = (ulong)&rad_c60_kepler },
	{ }
};
