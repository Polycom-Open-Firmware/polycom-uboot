			 ...
			 struct display_timing *timings, int bpp, bool bridge)
{
	...
	/*
	 * C60: the CCF path above can only divide the fixed 594 MHz video
	 * PLL (nearest to the panel's 79.61 MHz is /7 = 84.86 MHz, +6.6%
	 * fast — measured live; the DSI link budget is sized for exactly
	 * 79.61). Override with the raw setter, whose 79610 kHz special case
	 * reprograms the video PLL to 1194.15 MHz and divides by 15 for an
	 * exact match. Runs after the CCF calls so nothing re-touches it.
	 */
	mxs_set_lcdclk(priv->reg_base, timings->pixelclock.typ / 1000);
